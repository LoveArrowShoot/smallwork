...
{   //该形参为一个数组,每个数组元素为一个三维的向量
    ...
    //根据X1*Y2-X2*Y1可计算出叉乘后的Z值,以此来判断Z的方向是否一致
    if((AB[0] * AP[1] - AB[1] * AP[0] > 0
        ...
        && BC[0] * BP[1] - BC[1] * BP[0] < 0
        && CA[0] * CP[1] - CA[1] * CP[0] < 0))return 1;
    else return 0;
}
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    auto v = t.toVector4();
    float x1 = std::min(v[0][0], std::min(v[1][0], v[2][0]));
    float x2 = std::max(v[0][0], std::max(v[1][0], v[2][0]));
    float y1 = std::min(v[0][1], std::min(v[1][1], v[2][1]));
    float y2 = std::max(v[0][1], std::max(v[1][1], v[2][1]));
    //将该三角形的三个顶点的x,y最大,最小值分别向上,向下转换为整数
    x1 = (int)std::floor(x1);
    x2 = (int)std::ceil(x2);
    y1 = (int)std::floor(y1);
    y2 = (int)std::ceil(y2);
            ...
            for (int y = y1; y <=y2; y++) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (insideTriangle((float)x + pos[i][0], (float)y + pos[i][1], t.v)) {
                        ...
                        //根据一个像素内四个点的不同深度更新该像素的最小深度
                        minDepth = std::min(minDepth, z_interpolated);
                        count++;
                     //上述代码用于计算该点对应的深度
                    }
                }
                //计算完一个像素的四个小点后执行下面的条件语句
                if (count>0) {
                    //下述代码根据当前像素的插值与缓冲区对比的结果进行缓冲区的更新和像素颜色的修改
        ...
        for (int x = x1; x <= x2; x++) {
            for (int y = y1; y <= y2; y++) {
                if (insideTriangle((float)x + 0.5, (float)y + 0.5, t.v)) {
                    ...
                    z_interpolated *= w_reciprocal;
                    //上述代码用于计算该点对应的插值
                    //下述代码根据当前插值与缓冲区对比的结果进行缓冲区的更新和像素颜色的修改
